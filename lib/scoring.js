...
// Knockout wins still required to become champion, by furthest stage reached.
const REMAIN_KO_WINS = { none: 5, r32: 5, qf: 3, sf: 2, final: 1, champion: 0 };

// Best-case points a team could still ADD (advancement bonuses to champion +
// remaining knockout wins), excluding unknowable future goals. × multiplier.
function teamRemaining(tp) {
  const stage = tp.stage || 'none';
  const advLeft = cumulativeBonus('champion') - cumulativeBonus(stage);
  const koLeft = (REMAIN_KO_WINS[stage] ?? 0) * SCORING.knockout.win;
  return round2((advLeft + koLeft) * (tp.multiplier || 1));
}
...
export function compute(state, roster) {
  ...
  const standings = (roster || []).map((p) => {
    ...
    for (const pick of p.picks || []) {
      ...
      total += tp.total;
      remaining += teamRemaining(tp);
      breakdown.push(tp);
    }
    breakdown.sort((x, y) => y.total - x.total);
    return { name: p.name, total: round2(total), ceiling: round2(total + remaining), breakdown };
  });

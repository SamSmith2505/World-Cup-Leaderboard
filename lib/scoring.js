...
export function derivedAdvancement(state) {
  const rank = { r32: 1, qf: 2, sf: 3, final: 4, champion: 5 };
  const out = {};
  ...
  function bump(name, stage) {
    ...
    if (!out[canon] || rank[stage] > rank[out[canon]]) out[canon] = stage;
  }

  for (const m of consider) {
    if (!m || !m.teamA || !m.teamB) continue;
    // R32 and R16 both mean "advanced out of the group" (no separate R16 bonus).
    const stage = m.round === 'r32' || m.round === 'r16' ? 'r32'
      : m.round === 'qf' ? 'qf'
      : m.round === 'sf' ? 'sf'
      : m.round === 'final' ? 'final'
      : null;
  ...
  for (const m of state?.matches || []) {
    if (!m || m.round !== 'final' || !m.final) continue;
    const sa = Number(m.scoreA) || 0, sb = Number(m.scoreB) || 0;
    const w = sa > sb ? 'A' : sb > sa ? 'B' : (m.winner === 'A' || m.winner === 'B' ? m.winner : null);
    if (w === 'A') bump(m.teamA, 'champion');
    else if (w === 'B') bump(m.teamB, 'champion');
  }
  return out;
}

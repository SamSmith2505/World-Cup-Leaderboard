...
// Which ENTRIES are mathematically out of the contest (can't finish sole first),
// assuming a goal ceiling per remaining game. Returns a Set of player names.
//
// Two ideas combine into one clean test:
//  - Points only rise, so every entry's CURRENT total is a locked floor.
//  - With a goal cap, each surviving team has a finite max it can still add
//    (win out to champion, `goalCeil` goals/game, × tier, + unclaimed bonuses).
// Crucially, teams two entries SHARE score identically for both, so they cancel.
// Entry X can only pass Y using teams X has that Y doesn't. So X is eliminated
// if some Y already leads by more than X's best-case EXCLUSIVE upside:
//     current(Y) > current(X) + maxRemaining(X's surviving teams NOT owned by Y)
// This catches both "ceiling too low" and "picks are a subset of someone ahead".
export function contestEliminated(state, roster, { goalCeil = 5, standings } = {}) {
  const elim = eliminatedSet(state);
  const adv = derivedAdvancement(state);
  const perGame = SCORING.knockout.win + goalCeil * SCORING.knockout.goal;
  const gamesLeft = { champion: 0, final: 1, sf: 2, qf: 3, r16: 4, r32: 5 };
  const MAX_ADV = cumulativeBonus('champion');

  function teamRemaining(canon) {
    if (elim.has(canon)) return 0; // out -> no future points
    const stage = adv[canon];
    const games = gamesLeft[stage] ?? 5; // unknown stage but alive -> assume full run
    const remainingAdv = MAX_ADV - cumulativeBonus(stage || 'none');
    return (games * perGame + remainingAdv) * multiplierOf(canon);
  }

  const rows = standings || compute(state, roster).standings;
  const cur = Object.fromEntries(rows.map((s) => [s.name, s.total]));
  const owns = {};      // name -> Set(canonical picks)
  const surviving = {}; // name -> [canonical picks still alive]
  for (const p of roster || []) {
    const set = new Set((p.picks || []).map(canonicalTeam));
    owns[p.name] = set;
    surviving[p.name] = [...set].filter((t) => !elim.has(t));
  }
  ...
  for (const x of names) {
    for (const y of names) {
      if (x === y || cur[y] <= cur[x]) continue; // only a higher entry can dominate
      let exclusive = 0;
      for (const t of surviving[x] || []) if (!owns[y].has(t)) exclusive += teamRemaining(t);
      if (cur[y] > cur[x] + exclusive) { out.add(x); break; }
    }
  }
  return out;
}
